...
def Run(func):
    ...
    for i in range(TrialRuns):
        ...
        while curIter <= MaxIter:
            ...
            curIter += 1
            try:
                Best_list.append(FitCurrentBest[0])
            except Exception:
                Best_list.append(FitCurrentBest)
        All_Trial_Best.append(Best_list)
    np.savetxt("./DGTCIA_Data/Engineer/" + str(FuncNum) + ".csv", All_Trial_Best, delimiter=",")
...
def main():
    ...
    for i in range(len(Probs)):
        ...
        Run(Probs[i].evaluate)


if __name__ == "__main__":
    if os.path.exists('DGTCIA_Data/Engineer') == False:
        os.makedirs('DGTCIA_Data/Engineer')
    main()
